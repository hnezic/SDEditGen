...
{
    def program[_: P] : P[Program] = P( (title.? ~ objects ~ statements) . map { case (tit, obj, st) => Program(tit, obj, st) } )
    ...
    def arguments[_: P] : P[List[Argument]] = P( str("(") ~ argument.rep(sep=",").map(_.toList) ~ str(")") )
